...
def read_test_data(filename, numtypes):
  ...
  for row in xrange(book.nrows):
    time[row] = book.cell(row,0).value
    send_raw = book.cell(row,1).value
    sender[row] = send_raw[send_raw.find("<")+1:send_raw.find(">")].encode('ascii','ignore') # assumes <"name"> sender format
    subject[row] = (book.cell(row,2).translate(remove_punctuation_map)).encode('ascii','ignore').split()
    message[row] = (book.cell(row,3).translate(remove_punctuation_map)).encode('ascii','ignore').split()
